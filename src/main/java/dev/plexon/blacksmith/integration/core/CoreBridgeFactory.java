...
public final class CoreBridgeFactory {
    ...
    private static final String CORE_BRIDGE_CLASS = "dev.plexon.blacksmith.integration.core.PlexonCoreBridge";
    ...
    public static CoreBridge resolve(JavaPlugin plugin) {
        ...
        if (corePlugin == null) {
            plugin.getLogger().info("PlexonCore not installed; starting PlexonBlacksmith in standalone compatibility mode.");
        ...
        if (!corePlugin.isEnabled()) {
            plugin.getLogger().warning("PlexonCore is installed but disabled; using standalone compatibility mode.");
